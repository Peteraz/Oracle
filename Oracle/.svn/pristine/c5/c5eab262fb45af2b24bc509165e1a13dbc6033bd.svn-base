...
public class BaseDAO<T> {
	 ...
	 */	
	@Transactional
	public int getListByHQLN(final String hqlString,final List values) {
	 ...
	 */	
	@Transactional
	public List<T> getListByHQLL(final String hqlString,final int l) {
		Query query = gs().createQuery(hqlString);
	 ...
	 */	
	@Transactional
	public List<T> getListByHQL(final String hqlString, final List values,final int l) {
		Query query = gs().createQuery(hqlString);
		query.setMaxResults(l);           //限制查找数据条数
		if (values != null) {
			Object[] vvs = values.toArray(new Object[values.size()]);
	...
	public List<T> getListBySQL(final String sqlString, final List values) {
