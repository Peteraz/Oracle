...
public class ProgressAction extends BaseAction {
	...
	public void addProgress() throws Exception{
		...
		if(dao.have(Integer.parseInt(userId), type)==null)                                      //如果不存在通过的记录
		{
			Progress pro=new Progress();
			pro.setUserId(Integer.parseInt(userId));
			pro.setType(type);
			pro.setFrequency(Integer.parseInt(frequency));
			pro.setCreateTime(createTime);
			pro.setIsPass(Integer.parseInt(isPass));
			dao.add(pro);                                                //添加进度条记录
		}		
				
		User user=new User();
		user=udao.get(Integer.parseInt(userId));
		user.setType(type);
		udao.update(user);
